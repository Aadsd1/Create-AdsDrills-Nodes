...
public class AdsDrillRenderTypes extends RenderType {

    // RenderType 상속을 위한 필수 생성자
    private AdsDrillRenderTypes(String name, VertexFormat format, VertexFormat.Mode mode, int bufferSize, boolean affectsCrumbling, boolean sortOnUpload, Runnable setup, Runnable clear) {
        super(name, format, mode, bufferSize, affectsCrumbling, sortOnUpload, setup, clear);
    }

    // 1. 텍스처 경로 지정
    private static final ResourceLocation LASER_BEAM_TEXTURE = ResourceLocation.fromNamespaceAndPath(AdsDrillAddon.MOD_ID, "textures/effect/laser_beam.png");

    // 2. 커스텀 렌더 타입 정의
    public static final RenderType LASER_BEAM = create(
            AdsDrillAddon.MOD_ID + ":laser_beam",
    ...
    public static final RenderType NODE_SILHOUETTE = create(
            AdsDrillAddon.MOD_ID + ":node_silhouette",
    ...
    public static final RenderType NODE_SILHOUETTE_DEPTH_TESTED = create(
            AdsDrillAddon.MOD_ID + ":node_silhouette_depth_tested",
            DefaultVertexFormat.POSITION_COLOR,
            VertexFormat.Mode.LINES,
            256,
            false,
