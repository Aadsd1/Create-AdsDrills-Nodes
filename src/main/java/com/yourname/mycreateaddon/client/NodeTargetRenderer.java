...
public class NodeTargetRenderer {
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent event) {
        ...
        if (heldStack.getItem() != MyAddonItems.NETHERITE_NODE_LOCATOR.get()) {
            return;
        }
        ...
        VertexConsumer consumerDepthTested = bufferSource.getBuffer(MyAddonRenderTypes.NODE_SILHOUETTE_DEPTH_TESTED);
        LevelRenderer.renderLineBox(poseStack, consumerDepthTested, boundingBox, r, g, b, 0.4f); // 알파값 0.4

        // --- 2차 렌더링: 깊이 테스트 비활성화 (전체, 더 선명하게) ---
        VertexConsumer consumerNoDepth = bufferSource.getBuffer(MyAddonRenderTypes.NODE_SILHOUETTE);
        LevelRenderer.renderLineBox(poseStack, consumerNoDepth, boundingBox, r, g, b, 0.7f); // 알파값 0.7

        poseStack.popPose();

        // 각 렌더 타입에 대해 배치를 종료합니다.
        bufferSource.endBatch(MyAddonRenderTypes.NODE_SILHOUETTE_DEPTH_TESTED);
        bufferSource.endBatch(MyAddonRenderTypes.NODE_SILHOUETTE);
    }
}