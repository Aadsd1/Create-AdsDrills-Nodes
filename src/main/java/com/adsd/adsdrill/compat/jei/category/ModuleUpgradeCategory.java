...
import com.adsd.adsdrill.compat.jei.recipe.ModuleUpgradeRecipe;
...
public class ModuleUpgradeCategory implements IRecipeCategory<ModuleUpgradeRecipe> {
    ...
    public static final RecipeType<ModuleUpgradeRecipe> TYPE = RecipeType.create(AdsDrillAddon.MOD_ID, "module_upgrading", ModuleUpgradeRecipe.class);
    ...
    public ModuleUpgradeCategory(IGuiHelper helper) {
        ...
        icon = helper.createDrawableItemStack(new ItemStack(AdsDrillBlocks.FRAME_MODULE.get()));
    ...
    @Override public @NotNull Component getTitle() { return Component.translatable("adsdrill.jei.category.module_upgrading"); }
    ...
    public static List<ModuleUpgradeRecipe> getRecipes() {
        return ModuleUpgrades.getUpgradeResultMap().entrySet().stream()
                .map(entry -> new ModuleUpgradeRecipe(
                        new ItemStack(AdsDrillBlocks.FRAME_MODULE.get()),
